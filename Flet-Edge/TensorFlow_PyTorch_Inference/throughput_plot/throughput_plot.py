...
def plot_combine_bar(throughput_data_path, plot_combine_bar_list, combine_section_name):
    label_bar = []
    results_bar = []

    config_read_throughput_data = configparser.RawConfigParser()
    config_read_throughput_data.optionxform = lambda option: option
    config_read_throughput_data.read(throughput_data_path)
    ...
    for i in range(len_plot_combine_bar_list):
        ...
        model_name_i = config_read_throughput_data.get(section_name_i, "model_name")
        device_name_i = config_read_throughput_data.get(section_name_i, "device_name")
        input_size_i = config_read_throughput_data.getint(section_name_i, "input_size")
        throughput_i = config_read_throughput_data.getfloat(section_name_i, "throughput")
        label_bar.append(section_name_i)
        results_bar.append(throughput_i)
    ...
    plt.savefig(fname="TF_Py_combine_throughput_%s.png" % combine_section_name, format="png", bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    throughput_data_path = sys.argv[1]
    ...
    plot_combine_bar(throughput_data_path, plot_combine_bar_list, combine_section_name)
